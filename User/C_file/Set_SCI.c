...
////////////////////////////////////////////////////////////////////////////////////////////////
    SCI_setConfig(SCIB_BASE, DEVICE_LSPCLK_FREQ, 38400, (SCI_CONFIG_WLEN_8 |
                                                        SCI_CONFIG_STOP_ONE |
                                                        SCI_CONFIG_PAR_NONE));

    SCI_setFIFOInterruptLevel(SCIB_BASE, SCI_FIFO_TX1, SCI_FIFO_RX1); //FIFO 인터럽트 레벨 설정
    SCI_clearInterruptStatus(SCIB_BASE, SCI_INT_RXFF);
    SCI_resetRxFIFO(SCIB_BASE);
    SCI_resetTxFIFO(SCIB_BASE);
    SCI_resetChannels(SCIB_BASE);

    SCI_enableInterrupt(SCIB_BASE, SCI_INT_RXFF);    //RX FIFO Interrupt enable
